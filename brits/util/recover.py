...
def extract_all_sub_series(dataset):
    ...
    deltas_batch = torch.stack([sample['deltas'] for sample in batch])
    xi_batch = torch.stack([sample['xi'] for sample in batch])
    wi_batch = torch.stack([sample['wi'] for sample in batch])
    deltasi_batch = torch.stack([sample['deltasi'] for sample in batch])

    batch = {'t': t_batch, 'x': x_batch, 'w': w_batch, 'deltas': deltas_batch, 'xi': xi_batch,
             'wi': wi_batch, 'deltasi': deltasi_batch}
...
def create_batch_data(batch):
    ...
    rec['values'] = batch['x']
    rec['masks'] = batch['w']
    rec['deltas'] = batch['deltas']
    batch_data['forward'] = rec
    reci = {}
    reci['values'] = batch['xi']
    reci['masks'] = batch['wi']
    reci['deltas'] = batch['deltasi']
    batch_data['backward'] = reci

    return batch_data


# todo: overlap recover
