...
class MissingDataset(Dataset):
    ...
    def __init__(self, X, W, Wo, args):
        ...
        # self.delta = self.np2torch(delta)
    ...
    # def calculate_delta(self, mask):
    #     deltaPre = torch.zeros_like(mask, device=self.args.device)
    #     for i in range(1, deltaPre.shape[0]):
    #         deltaPre[i][mask[i - 1] == 0] = 1 + deltaPre[i - 1][mask[i - 1] == 0]
    #         deltaPre[i][mask[i - 1] == 1] = 1
    #
    #     return deltaPre

    @staticmethod
    def calculate_delta(mask):
        deltaPre = np.zeros_like(mask)
        for i in range(1, deltaPre.shape[0]):
            deltaPre[i][mask[i - 1] == 0] = 1 + deltaPre[i - 1][mask[i - 1] == 0]
            deltaPre[i][mask[i - 1] == 1] = 1
    ...
    def __getitem__(self, idx):
        ...
        delta = self.calculate_delta(w.cpu().data.numpy())
        delta = self.np2torch(delta)

        delta_inv = self.calculate_delta(w_inv.cpu().data.numpy())
        delta_inv = self.np2torch(delta_inv)
